...
fun BlockListSection(
    ...
    Column(
        ...
        TitledLazyList(
            ...
            title = stringResource(R.string.latest_training_block),
        ) {
            blocks.firstOrNull()?.let { latestBlockWithSessions ->
                item(latestBlockWithSessions) {
                    Button(
                        modifier = Modifier.fillMaxWidth(),
                        ...
                        DeletableItem(
                            ...
                            Column(
                                ...
                                Text(
                                    text =
                                    if (latestBlockWithSessions.sessions.isNotEmpty()) {
                                        stringResource(
        ...
        if (blocks.size > 1) {
            TitledLazyList(
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(TitleLazyListPadding),
                title = stringResource(R.string.preceding_training_blocks)
            ) {
