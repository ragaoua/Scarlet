...
class GetAllBlocksUseCaseTest {
    ...
    private var blocks: List<BlockWithSessions<Session>>
    ...
    init {
        runBlocking {
            ...
            blocks = getAllBlocks().first().data ?: emptyList()
        }
    }

    @Test
    fun blocksAreReturned() {
        assertTrue(blocks.isNotEmpty())
    }

    @Test
    fun blocksAreSortedByLatestSessionDescendingThenIdDescending() {
        ...
        assertTrue(areBlockSortedByLatestSessionDescendingThenIdDescending)
    }

    @Test
    fun sessionsAreSortedByDate() {
        blocks.forEach { block ->
            val areSessionsSortedByDate = block.sessions.isSortedBy { it.date }
            assertTrue(areSessionsSortedByDate)
        }
    }
}